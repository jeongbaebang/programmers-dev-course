...
const $serverStatusDisplay = (0, _utils.$)("#server-status");
const $list = (0, _utils.$)("aside .list");
const getDataAndRender = async ()=>{
    ...
    const createItem = (payload)=>{
        if (!payload) throw new Error("You must provide valid attributes.");
        return `
    <div class="item">
  ...
  `.trim();
    ...
    const createEmptyItem = ()=>{
        return `
    <div class="item">
      <p>\u{D68C}\u{C6D0} \u{B9AC}\u{C2A4}\u{D2B8}\u{AC00} \u{C874}\u{C7AC}\u{D558}\u{C9C0} \u{C54A}\u{C2B5}\u{B2C8}\u{B2E4}.</p>
    </div>
  `.trim();
    };
...
(async ()=>{
    try {
        await (0, _utils.delayExecution)(1500).start();
    ...
    // 폼 이벤트
    (0, _utils.$)("#send").addEventListener("click", (event)=>{
        event.preventDefault();
        // 폼 내부의 각 입력 값 가져오기
        const $name = (0, _utils.$)('.input__component[data-type="name"] > input');
        const $email = (0, _utils.$)('.input__component[data-type="email"] > input');
        const $address = (0, _utils.$)('.input__component[data-type="address"] > input');
        const $description = (0, _utils.$)('.input__component[data-type="description"] > input');
        const payload = {
            name: $name.value,
            email: $email.value,
...
parcelHelpers.export(exports, "delayExecution", ()=>delayExecution);
const $ = (tag)=>{
